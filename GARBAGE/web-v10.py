...
@app.route('/')
def index():
    return render_template('indexv10.html')
    ...
    while time.time() < end_time:
        if camera.IsGrabbing():
            ...
            if grab_result.GrabSucceeded():
                ...
                img_yuv = cv2.cvtColor(rotated_image, cv2.COLOR_BGR2YUV)
                img_yuv[:,:,0] = cv2.equalizeHist(img_yuv[:,:,0])
                equalized_image = cv2.cvtColor(img_yuv, cv2.COLOR_YUV2BGR)
                final_image = cv2.addWeighted(rotated_image, 1 - hist_eq_intensity, equalized_image, hist_eq_intensity, 0)
